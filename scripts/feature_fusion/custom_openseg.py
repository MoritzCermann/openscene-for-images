...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        print(f"Geladene Depth-Map-Größe: {depth.shape}, Max-Wert: {depth.max()}, Min-Wert: {depth.min()}")

        # calculate the 3d-2d mapping based on the depth
        mapping = np.ones([n_points, 4], dtype=int)
        mapping[:, 1:4] = point2img_mapper.compute_mapping(pose, locs_in, depth)
        print(f"Mapping-Shape: {mapping.shape}")
        print(f"Maximale X-Koordinate: {mapping[:, 1].max()}, Maximale Y-Koordinate: {mapping[:, 2].max()}")
        print(f"Image Dimensionen: {depth.shape}")
        if mapping[:, 3].sum() == 0: # no points corresponds to this image, skip
            continue

        # calculate the 3d-2d mapping based on the depth
        mapping = torch.from_numpy(mapping).to(device)
        print(f"Maximale X-Koordinate nach Konvertierung: {mapping[:, 1].max()}, Maximale Y-Koordinate: {mapping[:,2].max()}")
        mask = mapping[:, 3]
        vis_id[:, img_id] = mask
        if keep_features_in_memory:
            feat_2d = img_features[img_id].to(device)
        else:
            feat_2d = extract_openseg_img_feature(img_dir, openseg_model, text_emb, img_size=(1020, 768)).to(device)
            print(f"feat_2d Shape: {feat_2d.shape}")  # Sollte (C, 1020, 768) sein
...
def main(args):
    ...
    ##### Dataset specific parameters ##### Hochkant -> (768, 1020)
    img_dim = (768, 1020)
    depth_scale = 6000
